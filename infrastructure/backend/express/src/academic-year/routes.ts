...
academicYearRouter.get("/academic-years/current", ...authed(async (_req, res) => {
    ...
    respond(res, result, {
        not_found: { status: 404, error: "Aucune année académique en cours trouvée" },
...
academicYearRouter.get("/academic-years/:id", ...authed(async (req, res) => {
    ...
    respond(res, result, {
        not_found: { status: 404, error: "Année académique introuvable" },
        academic_year_found: (r) => ({ status: 200, body: r.academicYear }),
    });
}));
...
academicYearRouter.post("/academic-years", ...authed(async (req, res) => {
    ...
    respond(res, result, {
        ...
        academic_year_already_exists: { blocked: { type: "Creation", reason: "Une année académique avec ces dates existe déjà" } },
...
academicYearRouter.patch("/academic-years/:id", ...authed(async (req, res) => {
    ...
    respond(res, result, {
        not_found: { status: 404, error: "Année académique introuvable" },
        invalid_date_range: { status: 400, error: "startDate doit être antérieure à endDate" },
        academic_year_already_exists: { blocked: { type: "Operation", reason: "Une année académique avec ces dates existe déjà" } },
        academic_year_periods_out_of_range: { blocked: { type: "Operation", reason: "Certaines périodes sortiraient des nouvelles dates de l'année académique (ajustez-les ou déplacez-les d'abord)" } },
...
academicYearRouter.delete("/academic-years/:id", ...authed(async (req, res) => {
    ...
    respond(res, result, {
        not_found: { status: 404, error: "Année académique introuvable" },
        academic_year_is_current: { blocked: { type: "Operation", reason: "Impossible de supprimer l'année académique en cours" } },
        academic_year_has_periods: { blocked: { type: "Deletion", reason: "L'année académique a des périodes" } },
        academic_year_deleted: { status: 200, body: { message: "Année académique supprimée" } },
    });
}));
