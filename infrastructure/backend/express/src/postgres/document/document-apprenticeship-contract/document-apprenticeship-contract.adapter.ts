...
export const documentApprenticeshipContractRepository: DocumentApprenticeshipContractRepository = {
  async findById(id) {
    const result = await db.select().from(documentApprenticeshipContractTable).where(eq(documentApprenticeshipContractTable.id, id)).limit(1)
    return result[0] ? rowToDocumentApprenticeshipContract(result[0]) : undefined
  },
  async findByFileDocumentId(fileDocumentId) {
    const result = await db.select().from(documentApprenticeshipContractTable).where(eq(documentApprenticeshipContractTable.fileDocumentId, fileDocumentId)).limit(1)
    return result[0] ? rowToDocumentApprenticeshipContract(result[0]) : undefined
  },
  async findByCompanyId(companyId) {
    const result = await db.select().from(documentApprenticeshipContractTable).where(eq(documentApprenticeshipContractTable.companyId, companyId))
  ...
  async list() {
    const result = await db.select().from(documentApprenticeshipContractTable).orderBy(asc(documentApprenticeshipContractTable.startDate))
    return result.map(rowToDocumentApprenticeshipContract)
  },
}
