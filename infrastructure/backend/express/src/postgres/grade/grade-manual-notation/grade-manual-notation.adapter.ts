...
export const gradeManualNotationRepository: GradeManualNotationRepository = {
  async findById(id) {
    const result = await db.select().from(gradeManualNotationTable).where(eq(gradeManualNotationTable.id, id)).limit(1)
    return result[0] ? rowToGradeManualNotation(result[0]) : undefined
  },
  async findByGradeId(gradeId) {
    const result = await db.select().from(gradeManualNotationTable).where(eq(gradeManualNotationTable.gradeId, gradeId))
    return result.map(rowToGradeManualNotation)
  },
  async findByGradeManualId(gradeManualId) {
    const result = await db.select().from(gradeManualNotationTable).where(eq(gradeManualNotationTable.gradeManualId, gradeManualId))
    return result.map(rowToGradeManualNotation)
  },
