...
import { db } from "../../db"
import { gradeAssessment as gradeAssessmentTable } from "../../schema/grade"
...
export const gradeAssessmentRepository: GradeAssessmentRepository = {
  ...
  async findByAssessmentId(assessmentId) {
    const result = await db.select().from(gradeAssessmentTable).where(eq(gradeAssessmentTable.assessmentId, assessmentId))
    return result.map(rowToGradeAssessment)
  },
