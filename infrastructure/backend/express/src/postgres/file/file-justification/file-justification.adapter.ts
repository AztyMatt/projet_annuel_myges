...
export const fileJustificationRepository: FileJustificationRepository = {
  ...
  async findByAbsenceId(absenceId) {
    const result = await db.select().from(fileJustificationTable).where(eq(fileJustificationTable.absenceId, absenceId))
    return result.map(rowToFileJustification)
  },
  async findByFileId(fileId) {
    const result = await db.select().from(fileJustificationTable).where(eq(fileJustificationTable.fileId, fileId)).limit(1)
    return result[0] ? rowToFileJustification(result[0]) : undefined
  },
