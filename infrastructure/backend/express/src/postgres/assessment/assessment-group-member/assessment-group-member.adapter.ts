...
import { db } from "../../db"
import { assessmentGroupMember as assessmentGroupMemberTable } from "../../schema/assessment"
...
export const assessmentGroupMemberRepository: AssessmentGroupMemberRepository = {
  async findById(id) {
    const result = await db.select().from(assessmentGroupMemberTable).where(eq(assessmentGroupMemberTable.id, id)).limit(1)
    return result[0] ? rowToAssessmentGroupMember(result[0]) : undefined
  },
  async findByAssessmentGroupId(assessmentGroupId) {
    const result = await db.select().from(assessmentGroupMemberTable).where(eq(assessmentGroupMemberTable.assessmentGroupId, assessmentGroupId))
    return result.map(rowToAssessmentGroupMember)
  },
  async findByStudentId(studentId) {
    const result = await db.select().from(assessmentGroupMemberTable).where(eq(assessmentGroupMemberTable.studentId, studentId))
    return result.map(rowToAssessmentGroupMember)
  },
