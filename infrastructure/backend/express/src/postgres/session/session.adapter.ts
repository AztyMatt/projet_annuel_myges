...
export const sessionRepository: SessionRepository = {
  ...
  async findByCourseId(courseId) {
    const result = await db.select().from(sessionTable).where(eq(sessionTable.courseId, courseId)).orderBy(asc(sessionTable.startTime))
    return result.map(rowToSession)
  },
  async findByClassroomId(classroomId) {
    const result = await db.select().from(sessionTable).where(eq(sessionTable.classroomId, classroomId)).orderBy(asc(sessionTable.startTime))
    return result.map(rowToSession)
  },
  async save(session) {
    await db
