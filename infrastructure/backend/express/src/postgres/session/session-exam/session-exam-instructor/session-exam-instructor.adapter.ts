...
export const sessionExamInstructorRepository: SessionExamInstructorRepository = {
    async findById(id) {
        const result = await db
            ...
            .where(eq(sessionExamInstructorTable.id, id))
            .limit(1);
    ...
    async findBySessionExamId(sessionExamId) {
        const result = await db
            .select()
            .from(sessionExamInstructorTable)
            .where(eq(sessionExamInstructorTable.sessionExamId, sessionExamId));
    ...
    async existsByInstructorId(instructorId) {
        const rows = await db.select({ id: sessionExamInstructorTable.id }).from(sessionExamInstructorTable).where(eq(sessionExamInstructorTable.instructorId, instructorId)).limit(1);
