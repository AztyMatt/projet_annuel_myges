...
import { db } from "../../../db"
import { sessionExamStudent as sessionExamStudentTable } from "../../../schema/session"
...
export const sessionExamStudentRepository: SessionExamStudentRepository = {
  async findById(id) {
    const result = await db.select().from(sessionExamStudentTable).where(eq(sessionExamStudentTable.id, id)).limit(1)
    return result[0] ? rowToSessionExamStudent(result[0]) : undefined
  },
  async findBySessionExamId(sessionExamId) {
    const result = await db.select().from(sessionExamStudentTable).where(eq(sessionExamStudentTable.sessionExamId, sessionExamId))
    return result.map(rowToSessionExamStudent)
  },
  async findByStudentId(studentId) {
    const result = await db.select().from(sessionExamStudentTable).where(eq(sessionExamStudentTable.studentId, studentId))
    return result.map(rowToSessionExamStudent)
  },
