...
sessionExamRouter.post("/session-exams", ...authed(async (req, res) => {
    ...
    respond(res, result, {
        missing_fields: { status: 400, error: "sessionId and type are required" },
...
sessionExamRouter.get("/session-exam-students/session-exam/:sessionExamId", ...authed(async (req, res) => {
    const result = await sessionExamUseCases.listStudentsBySessionExam(String(req.params.sessionExamId));
    send(res, { status: 200, body: result.sessionExamStudents });
}));
...
sessionExamRouter.post("/session-exam-students", ...authed(async (req, res) => {
    ...
    respond(res, result, {
        missing_fields: { status: 400, error: "sessionExamId and studentId are required" },
...
sessionExamRouter.delete("/session-exam-students/:id", ...authed(async (req, res) => {
    ...
    respond(res, result, {
        not_found: { status: 404, error: "Session exam student not found" },
        session_exam_already_started: { blocked: { type: "Operation", reason: "Session exam has started" } },
        session_exam_has_grades: { blocked: { type: "Deletion", reason: "This student already has a grade for this exam" } },
...
sessionExamRouter.get("/session-exam-instructors/session-exam/:sessionExamId", ...authed(async (req, res) => {
    const result = await sessionExamUseCases.listInstructorsBySessionExam(String(req.params.sessionExamId));
    send(res, { status: 200, body: result.sessionExamInstructors });
}));

sessionExamRouter.get("/session-exam-instructors/instructor/:instructorId", ...authed(async (req, res) => {
    const result = await sessionExamUseCases.listSessionExamsByInstructor(String(req.params.instructorId));
    send(res, { status: 200, body: result.sessionExamInstructors });
}));
...
sessionExamRouter.post("/session-exam-instructors", ...authed(async (req, res) => {
    ...
    respond(res, result, {
        missing_fields: { status: 400, error: "sessionExamId and instructorId are required" },
...
sessionExamRouter.get("/session-exam-externals/session-exam/:sessionExamId", ...authed(async (req, res) => {
    const result = await sessionExamUseCases.listExternalsBySessionExam(String(req.params.sessionExamId));
    send(res, { status: 200, body: result.sessionExamExternals });
}));
...
sessionExamRouter.post("/session-exam-externals", ...authed(async (req, res) => {
    ...
    respond(res, result, {
        missing_fields: { status: 400, error: "sessionExamId and externalId are required" },
