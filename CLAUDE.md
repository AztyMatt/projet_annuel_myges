...
- [ ] Signalement des erreurs (Sentry / GlitchTip) — aucun intégré → `OBS-002`
- [~] **Analytique RGPD — Umami auto-hébergé déployé (2026-07-14) mais pas encore branché** : `k8s/observability/umami/` (deployment + postgres dédié avec PVC, secrets via `INFISICAL_TOKEN_ROOT` injecté par le CD, IngressRoute + Certificate). ⚠️ **Le script de tracking Umami n'est intégré nulle part dans le front** (vérifié : aucune référence dans `infrastructure/frontend/next/`) — Umami tourne mais ne collecte rien. Reste : injecter le `<script>` dans `app/layout.tsx` (URL + website-id via variable d'env) → fin de `OBS-003`
...
- [~] **Résolution nom/prénom à partir d'un `userId`** — `GET /users/:id` existe (2026-07-12). Appliqué sur `/intervenant/notes`, `/messagerie` (expéditeur, liste des conversations privées admin, sélecteur "Nouvelle conversation"), `/scolarite/etudiants` (2026-07-12) et `/superadmin/securite` (2026-07-15, colonne "Utilisateur" du journal d'audit — même pattern que `/scolarite/intervenants` : résolution en lot des `userId` uniques de la page au chargement, mise en cache dans un `Record`, repli sur `Utilisateur #{id}` si `GET /users/:id` échoue ; le filtre "Utilisateur" matche désormais le nom résolu en plus de l'id brut). Reste à appliquer page par page : `/students/:id` (roster hors instructeur concerné), `/instructors/:id`, `/courses/*`, etc.
...
- [x] **`/messagerie`** — reconnectée
  ...
  - **Polling sur la conversation active (2026-07-15)** — même décision d'équipe que les notifications (pas de WebSocket, cf. section 10) : `GET /messages/conversation/:id` est réinterrogé toutes les 30s (`POLL_INTERVAL_MS`) tant qu'une conversation est ouverte, avec diff côté client (`messagesRef`) pour ne marquer "lu"/résoudre les noms d'expéditeur que sur les messages réellement nouveaux, pas sur tout le fil à chaque tick. Auto-scroll vers le bas quand de nouveaux messages arrivent. Les échecs de poll (silencieux, `opts.silent`) n'effacent pas le fil déjà affiché, contrairement à l'échec du chargement initial. **Limitation assumée** : aucun endpoint backend n'expose de compteur de messages non lus (par conversation ou global, contrairement à `GET /notifications/mine/unread-count`) — la liste des conversations dans la sidebar ne se rafraîchit donc pas toute seule et n'affiche pas de badge "nouveau message" ; seul le fil de la conversation ouverte est mis à jour en direct. À étendre si un badge est demandé (nécessiterait un nouvel endpoint agrégé côté backend)
